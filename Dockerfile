...
ADD hook-bootkit .
# ADD mlxbf-bootimages-signed-4.0.3-12704.aarch64.rpm .
# ADD mlnx-ofa_kernel-23.04-3.cm2.aarch64.rpm .
# ADD kernel-5.15.111.1-1.cm2.aarch64.rpm .
# ADD kernel-devel-5.15.111.1-1.cm2.aarch64.rpm .
# ADD kernel-headers-5.15.111.1-1.cm2.noarch.rpm .
...
# RUN yum-config-manager --dump-variables
...
# RUN rpm -ihv --force --nodeps kernel-*.rpm || true
# RUN rpm -ihv --force --nodeps kernel-devel-5.15.111.1-1.cm2.aarch64.rpm || true
# RUN rpm -ihv --force --nodeps kernel-headers-5.15.111.1-1.cm2.aarch64.rpm || true
...
# RUN yum install -y ibacm ibutils2 infiniband-diags infiniband-diags-compat libibumad libibverbs-utils librdmacm librdmacm-utils libxpmem libxpmem-devel mft mft-oem mlnx-ethtool mlnx-fw-updater mlnx-iproute2 mlnx-libsnap mlx-regex mlx-bootctl mlxbf-bootctl mstflint ofed-scripts opensm opensm-devel opensm-libs opensm-static rdma-core rdma-core-devel srp_daemon ucx ucx-cma ucx-devel ucx-ib ucx-knem ucx-rdmacm xpmem mlnx-tools mlnx-dpdk mlnx-dpdk-devel dpcp libvma libvma-utils python3-grpcio python3-protobuf rxp-compiler  mlxbf-bfscripts
...
# RUN rpm -iv --nodeps mlnx-ofa_kernel-23.04-3.cm2.aarch64.rpm || true
# RUN /bin/rm -f /etc/infiniband/vf-net-link-name.sh
# RUN ex cp /usr/share/doc/mlnx-ofa_kernel/vf-net-link-name.sh /etc/infiniband/vf-net-link-name.sh
# RUN ex chmod +x /etc/infiniband/vf-net-link-name.sh
# RUN /bin/rm -f /etc/udev/rules.d/82-net-setup-link.rules
# RUN cp /usr/share/doc/mlnx-ofa_kernel/82-net-setup-link.rules /etc/udev/rules.d/82-net-setup-link.rules
